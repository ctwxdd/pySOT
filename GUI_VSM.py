...
# Note: Modified for PC DeLorean (attempted for VSM setup)
# Note: Keithley2400 function is temporarily voided
...
def measureMethod( _inteval, _number, _output, _average, _signal, _frequency, _current):
    ...
    ax.clear()
    ax.grid(True)
    ax.set_title("Realtime Lock-In voltage vs H Plot")
    ...
    def event():
        ...
        while t < n :
            ...
            values_y.append(tmp)
            values_x.append(a*i)
            ax.plot(values_x, values_y, 'b-o', alpha=0.5)
            canvas.draw()
...
def keithOutMethod(val):
    ...
    print('This function is voided now.')
#    
#    keith=Keithley2400(f)
#    
#    if val =='ON':
...
def createWidgit():
    ...
    entry_interval = ttk.Entry(frame_setting);entry_interval.insert(0,"5256.2")
