...
def main():
    ...
    sense='20mV' #Set a default sensitivity range (mV or uV)
    ...
    freq=117 #Set a default OSC frequency (Hz)

    read_me = 'This program uses Keithley2400 to supply current to the sample. The Lock-in Amp generates Hx and Hz fields along with a signal OSC voltage. \
    The Lock-in Amp also measures the voltage of the device.'
...
def measureMethod(_interval, _number, _output, _average, _signal, _frequency, _current, _step, _Hx, _dHx, _intervalx, _sample):
    ...
    signal=float(_signal) # Lock-in Amp output signal voltage (Max 5)
    freq=int(_frequency) # Lock-in Amp signal frequency
    ...
    def event():
        ...
        while Hx_start>=Hx_end:
            ...
            current_start=float(_current) # Max current set by Keithley  2400
            current_end=float(_current)*(-1) # Min current set by Keithley 2400
            ...
            while current_start>=current_end:
                ...
                file = open(str(directory)+"/"+str(_sample)+"_"+str(save)+"Ohm_"+str(Hx_start)+"Oe_"+str(current_start)+"mA_"+str(stamp), "w")
...
def createWidgit():
    ...
    entry_frequency = ttk.Entry(frame_setting); entry_frequency.insert(0,"117")
    ...
    mode = ["1st","1st","2nd"]
    sensitivity = ["20mV","1mV","2mV","5mV","10mV","20mV","50mV","100mV","200mV","10uV","20uV","50uV","100uV"]
    dac = [2,1,2,3,4]
    dacx = [3,1,2,3,4]
